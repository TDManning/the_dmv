...
RSpec.describe FacilityFactory do
  ...
  describe '#verify data NY' do
    it 'show facilities with specific values at NY DMV' do
      ...
      expect(new_york_facilities[0].collected_fees).to be(0)
      expect(new_york_facilities[0].registered_vehicles).to be_an_instance_of(Array)
      expect(new_york_facilities[0].services).to be_an_instance_of(Array)
    end
  end 
  ...
  describe '#verify data MO' do
    it 'show facilities with specific values at MO DMV' do
      ...
      expect(missouri_facilities[0].collected_fees).to be(0)
      expect(missouri_facilities[0].name).to be_a(String)
      expect(missouri_facilities[0].phone).to be_a(String)
      expect(missouri_facilities[0].registered_vehicles).to be_an_instance_of(Array)
      expect(missouri_facilities[0].services).to be_an_instance_of(Array)
    end
  end 

#CO DMV API CURRENTLY "ERROR"/SUSPENDED FOR USE
